...
The 1,440px container also stretches short operational content across a very wide scan path. This amplifies the small-text impression and leaves the left workspace visually sparse next to the result plane.

## Considered approaches

1. **Balanced editorial type system — selected.** Reduce the thesis to a 36–40px desktop range, establish 16px ordinary copy, raise operational labels and metadata to 14–15px, use 18px numeric values, and slightly reduce the desktop reading width. This fixes the ratio at its source and preserves the approved visual identity.
...
- Reduce the maximum desktop container from 1,440px to 1,360px so operational labels do not float across excessive horizontal distance.
- Preserve the approved 8/4 workspace, square geometry, unboxed inputs, single tonal result plane, and responsive stacking behavior.
