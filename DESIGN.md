...
The desktop container is fluid up to 1600px wide, allowing a 1908px portfolio viewport to use the available stage without becoming edge-to-edge. The first viewport follows a 12-column editorial composition: thesis and content-sized scope boundary, four equal unboxed KPI pairs, then a 7/5 input-to-result workspace that gives result metadata a comfortable reading width. The input plane is one complete audit worksheet: a compact heading and case loader share the toolbar; all four canonical feature groups remain visible as touching rows; and each row pairs one label rail with a five- or six-track field ledger. Equal field cells use consistent left insets and internal hairline separators. The case note and primary action share one immediate footer instead of floating below the fields. Inside the result plane, calibrated probability and model metadata share one content-driven facts row on non-phone layouts; below 520px they stack in source order. The workspace height follows useful content instead of reserving filler space, and the evidence transition remains within 24px. Successful attributions occupy a conditional full-width evidence band below the workspace; empty and error states reserve no table space. At the 820px compact breakpoint, columns stack in source order, KPIs become a 2-by-2 grid, and feature ledgers use three equal columns; below 520px, group headings become horizontal strips and fields use two equal columns without page-level horizontal scrolling. The primary action is compact and right-aligned on wider screens, then full-width only on narrow phones. Center only short, equal-weight content such as status labels, KPI pairs, compact controls, and the primary action. Keep thesis copy, explanatory prose, group and form labels, field values, model metadata, attributions, and evidence rows left-aligned.
...
- Case workspace: one compact toolbar followed by four always-visible shared-rule group ledgers containing all 23 integer-only inputs in canonical model order, then an immediate case-context/action footer.
